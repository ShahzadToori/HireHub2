...
router.get('/', requireAdmin, async (req, res) => {
  try {
    ...
    const [[{ total }]] = await db.execute(`SELECT COUNT(*) AS total FROM messages ${whereSql}`, params);
    ...
    const [[{ unread }]] = await db.execute("SELECT COUNT(*) AS unread FROM messages WHERE status = 'unread'");
...
router.get('/:id', requireAdmin, async (req, res) => {
  try {
    const [rows] = await db.execute('SELECT * FROM messages WHERE id = ? LIMIT 1', [req.params.id]);
