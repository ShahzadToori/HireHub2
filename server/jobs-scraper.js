/* ══════════════════════════════════════════════════════════
   jobs-scraper.js
   Sources: JSearch API + Adzuna API (GB with Saudi filter)
   Schedule: Daily at midnight via PM2
   Run manually: node server/jobs-scraper.js
══════════════════════════════════════════════════════════ */
...
async function getCategoryId(title) {
  ...
  for (const cat of cats) {
    const kws = cat.slug.replace(/-/g,' ').split(' ')
      .concat(cat.name.toLowerCase().split(/[\s\/]+/));
    let score = 0;
    for (const kw of kws) { if (kw.length > 2 && t.includes(kw)) score++; }
    if (score > bestScore) { bestScore = score; bestId = cat.id; }
  }
  if (bestScore === 0) {
    const o = cats.find(c => c.slug.includes('other') || c.slug.includes('specialist'));
    if (o) bestId = o.id;
  }
  return bestId;
}
...
async function saveJob(title, company, location, description, applyLink, jobType, salary) {
  ...
  await db.query(
    `INSERT INTO jobs
     (title,company,category_id,location,job_type,description,apply_link,salary,status,slug,created_at,updated_at)
     VALUES (?,?,?,?,?,?,?,?,'active',?,NOW(),NOW())`,
    [title, company, catId,
     (location || 'Saudi Arabia').slice(0, 200),
     jobType || 'full-time',
     (description || '').slice(0, 5000),
     applyLink || '', salary || '', slug]
  );
  return true;
}

/* ════════════════════════════════════════════════════════
   SOURCE 1 — JSearch API (Google Jobs aggregator)
   Free: 200 req/month | 6 queries = ~180/month
════════════════════════════════════════════════════════ */
const JSEARCH_QUERIES = [
  'safety officer HSE officer fire watcher hole watch Saudi Arabia',
  'QC inspector quality control NDT welding inspector Saudi Arabia',
  'WPR permit receiver work permit Saudi Arabia construction',
  'welder pipe welder structural welder fabricator pipe fitter Saudi Arabia',
  'rigger scaffolder crane operator forklift operator Saudi Arabia',
  'construction supervisor foreman general foreman Saudi Arabia',
];

async function runJSearch() {
  if (!JSEARCH_KEY) { console.log('[JSearch] No key — skipping'); return 0; }
  let saved = 0;
  for (const query of JSEARCH_QUERIES) {
    try {
      ...
      if (!res.ok) { console.error('[JSearch] HTTP Error:', res.status); continue; }
      ...
      for (const j of (data.data || [])) {
        ...
        const ok = await saveJob(
          j.job_title, j.employer_name,
          j.job_city || j.job_state,
          j.job_description, j.job_apply_link, type, salary
        );
        if (ok) saved++;
      }
...
/* ════════════════════════════════════════════════════════
   SOURCE 2 — Adzuna API (GB endpoint, Saudi keyword filter)
   Free: 250 req/day | Saudi Arabia not natively supported
   Filter: only save jobs mentioning Saudi Arabia
════════════════════════════════════════════════════════ */
const ADZUNA_QUERIES = [
  'safety officer HSE Saudi Arabia',
  'QC inspector Saudi Arabia',
  'permit receiver WPR Saudi Arabia',
  'construction supervisor Saudi Arabia',
  'civil engineer Saudi Arabia',
  'scaffolding rigger Saudi Arabia',
  'welder fabricator Saudi Arabia',
  'site engineer Saudi Arabia',
  'NDT inspector Saudi Arabia',
  'electrical technician Saudi Arabia',
];

async function runAdzuna() {
  if (!ADZUNA_ID || !ADZUNA_KEY) { console.log('[Adzuna] No keys — skipping'); return 0; }
  let saved = 0;
  for (const query of ADZUNA_QUERIES) {
    try {
      ...
      if (!res.ok) { console.error('[Adzuna] HTTP Error:', res.status); continue; }
      const data = await res.json();
      for (const j of (data.results || [])) {
        // Only save jobs that mention Saudi Arabia in title or description
        const text = ((j.title || '') + ' ' + (j.description || '')).toLowerCase();
        if (!text.includes('saudi')) continue;
        const ok = await saveJob(
          j.title,
...
async function run() {
  ...
  console.log(`[Scraper] Done — JSearch: ${j} | Adzuna: ${a} | Total: ${j + a} new jobs`);
  process.exit(0);
}

run().catch(e => { console.error('[Scraper] Fatal:', e.message); process.exit(1); });
