...
**WhatsApp bot reconnect** (`admin/whatsapp-settings.html`, `server/routes/admin-whatsapp.js`, menu key `whatsapp`): a web UI for the manual SSH reconnect procedure (wipe `cron-session/`, run `pair-link.js` for a pairing code, wait for link). `POST /api/admin/whatsapp/reconnect/start` wipes the session and spawns `xvfb-run -a node pair-link.js <phone>` from Node; progress (pairing code, linked/failed/timeout) is written to `channel-forwarder/.reconnect-state.json` and polled by the page, **not** kept in server memory — PM2 runs the app in cluster mode, so the request that starts a reconnect and the ones that poll it can land on different workers. The "only one reconnect at a time" guard is an atomic `fs.writeFileSync(lockfile, ..., {flag:'wx'})` (throws `EEXIST` if already locked), not a read-then-write check — a read-then-write version race-loses against the async session-store lookup that runs before every route handler. The same lock file (`channel-forwarder/.reconnect.lock`) is checked by `run-smart.sh` (skips its run if present) so the 5-minute cron tick can't collide with an in-progress pairing session; `GET /status` self-heals a lock older than 6 minutes (pair-link.js's own timeout) in case the app restarted mid-reconnect and orphaned it. Connection status shown on the page is inferred by tailing `/var/log/smart-cron.log` for the last occurrence of a ready/fail marker — there's no persistent "connected" flag anywhere, `smart-cron.js` itself was intentionally not touched (see operational rule below).
...
- WhatsApp pipeline: VPS crontab runs `run-smart.sh` every 5 minutes → `smart-cron.js`. Session lives in `cron-session/`. Relink via **admin → WhatsApp Bot** (`admin/whatsapp-settings.html`) instead of SSH where possible — it wraps the same `pair-link.js` pairing-code flow (see Architecture above). Manual fallback: `rm -rf cron-session && xvfb-run -a node pair-link.js <phone-number>`. `run-smart.sh` now has a lock-file guard (added 2026-08-07, approved) and per-run timestamp markers in `/var/log/smart-cron.log`; do NOT modify smart-cron.js or the crontab schedule itself without explicit approval.
